...
for (const n of needles) {
  ...
  console.log(n, idx);
  if (idx < 0) continue;
  const slice = html.slice(Math.max(0, idx - 2000), idx + 1500);
  const urls = [
...
console.log("\nFound", found.length);
found.forEach((u, i) => console.log(i, u));

let i = 0;
for (const url of found.slice(0, 10)) {
  const ext = path.extname(new URL(url).pathname) || ".webp";
  await download(url, path.join(outDir, `member-${i}${ext}`));
  console.log("saved", i, ext);
  i++;
}
